...
ARPFrame_t MakeARP(pcap_addr* a) {
	...
	//获取远程IP与MAC对应时应该将将ARPFrame.SendIP设置为本机网卡上绑定的IP地址
	ARPFrame.SendIP = inet_addr("100.100.100.100");
	//获取远程IP与MAC对应时应该将将ARPFrame.RecvHa设置为0
	for (int i = 0; i < 6; i++)
		ARPFrame.RecvHa[i] = 0;//表示目的地址未知
	//获取远程IP与MAC对应时应该将将ARPFrame.RecvIP设置为请求的IP地址
...
int Send(pcap_t* adhandle, ARPFrame_t ARPFrame) {
	if (int n = pcap_sendpacket(adhandle, (u_char*)&ARPFrame, sizeof(ARPFrame_t)) != 0) {  return 1; }// !=0的时候为send发生错误
	else { return 1; }
}
